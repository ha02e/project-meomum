...
public class SvcController {
	...
	@ResponseBody
	public ModelAndView svcSearch(@RequestParam("minDate") String minDate, @RequestParam("maxDate") String maxDate,
			@RequestParam(value = "category", defaultValue = "1") int category, @RequestParam("keyword") String keyword,
			@RequestParam("state[]") List<String> state, @RequestParam(value = "cp", defaultValue = "1") int cp) {
	...
	public ModelAndView svcIngInsert(SvcIngDTO dto) {
		...
		mav.addObject("gopage","opener.location.reload(); self.close();");
	...
	public ModelAndView asvcIngUpdate(SvcIngDTO dto, SvcMemDTO memDto) {
		...
		mav.addObject("msg", "서비스 희망일 및 견적이 수정되었습니다");
		mav.addObject("gopage", "opener.location.reload(); self.close();");

		/*
		 * boolean result = ingUpdate >0 && memUpdate >0;
		 * 
		 * if(result=true) {
		 * 
		 * mav.addObject("msg","서비스 희망일 및 견적이 수정되었습니다"); mav.addObject("gopage",
		 * "opener.location.reload(); self.close();");
		 * 
		 * }else { mav.addObject("msg","다시 시도해주세요"); mav.addObject("gopage",
		 * "asvcIngPopup.do"); }
		 */
		mav.setViewName("mainMsg");
		return mav;
