...
public class ShippingController {
	...
	public ModelAndView shipInsert(ShippingDTO dto,
		...
		int order_amount=orderDao.orderData(order_idx).getPro_amount();
		...
		if(order_amount>=origin_amount) {
			...
			int amountUpdateOk=proDao.proAmountShipUpdate(order_idx); //배송처리 성공 시 상품 재고수량 수정
			
			if(result>0 && amountUpdateOk>0) {
				int statusUpdateOk=orderDao.shipStartUpdate(order_idx); //배송처리 성공 시 주문테이블 상태 2로 수정
